...
class SBFailureDetectorStateSuite extends WordSpec with Matchers {
  ...
  "SBRFailureDetectorState" must {
    ...
    "be unreachable when there is no contention" in {
      val s = SBFailureDetectorState(testPath(aa)).unreachable(aa, bb)
      s.updatedStatus(bb)._1 should ===(Some(Unreachable))

      val s1 = SBFailureDetectorState(testPath(aa)).reachable(bb).unreachable(aa, bb)
      s1.updatedStatus(bb)._1 should ===(Some(Unreachable))
    }

    "be indirectly connected when there is a contention" in {
      val s = SBFailureDetectorState(testPath(aa)).contention(bb, cc, dd, 1)
      s.updatedStatus(dd)._1 should ===(Some(IndirectlyConnected))
    }

    "be unreachable when a contention is resolved" in {
      val s = SBFailureDetectorState(testPath(aa)).contention(aa, cc, dd, 1).unreachable(aa, dd)
    ...
    "be unreachable only when all the contentions are resolved" in {
      val s =
        SBFailureDetectorState(testPath(aa))
          .contention(cc, aa, bb, 1)
          .contention(cc, dd, bb, 1)
          .contention(ee, aa, bb, 1)
    ...
    "ignore a contention for old versions" in {
      val s = SBFailureDetectorState(testPath(aa)).contention(ee, aa, bb, 2).contention(cc, aa, bb, 1)
    ...
    "reset a contention when there is a new version" in {
      val s = SBFailureDetectorState(testPath(aa)).contention(cc, aa, bb, 1).contention(ee, aa, bb, 2)
    ...
    "update a contention when it is for the current version" in {
      val s = SBFailureDetectorState(testPath(aa)).contention(cc, aa, bb, 1).contention(ee, aa, bb, 1)
    ...
    "become reachable after calling reachable" in {
      val s =
        SBFailureDetectorState(testPath(aa)).unreachable(aa, bb).contention(aa, bb, cc, 1).contention(dd, bb, cc, 1)
    ...
    "update contentions when a node is removed" in {
      val s =
        SBFailureDetectorState(testPath(aa))
          .contention(aa, bb, cc, 1)
          .contention(aa, dd, bb, 2)
          .contention(dd, cc, aa, 1)
