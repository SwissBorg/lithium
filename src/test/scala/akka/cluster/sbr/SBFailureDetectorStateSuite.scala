...
class SBFailureDetectorStateSuite extends WordSpec with Matchers {
  ...
  "SBRFailureDetectorState" must {
    "be reachable when empty" in {
      val s = SBFailureDetectorState.empty
    ...
    "ignore a contention for old versions" in {
      val s = SBFailureDetectorState.empty.contention(ee, aa, bb, 2).contention(cc, aa, bb, 1)
    ...
    "reset a contention when there is a new version" in {
      val s = SBFailureDetectorState.empty.contention(cc, aa, bb, 1).contention(ee, aa, bb, 2)
    ...
    "update a contention when it is for the current version" in {
      val s = SBFailureDetectorState.empty.contention(cc, aa, bb, 1).contention(ee, aa, bb, 1)
    ...
    "become reachable after calling reachable" in {
      val s = SBFailureDetectorState.empty.unreachable(aa, bb).contention(aa, bb, cc, 1).contention(dd, bb, cc, 1)
    ...
    "update contentions when a node is removed" in {
      val s =
        SBFailureDetectorState.empty.contention(aa, bb, cc, 1).contention(aa, dd, bb, 2).contention(dd, cc, aa, 1)
