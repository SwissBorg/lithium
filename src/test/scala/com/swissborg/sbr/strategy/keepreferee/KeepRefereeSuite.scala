...
import com.swissborg.sbr.strategy.keepreferee.KeepReferee.Config.{Address => RefereeAddress}
...
class KeepRefereeSuite extends WordSpec with Matchers {
  ...
  private val referee =
    refineV[RefereeAddress](aa.address.toString).left.map(new IllegalArgumentException(_)).toTry.get
