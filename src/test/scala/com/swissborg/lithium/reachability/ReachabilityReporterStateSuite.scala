...
class ReachabilityReporterStateSuite extends WordSpec with Matchers {
  ...
  "ReachabilityReporterState" must {
    "ignore members from the same DC" in {
      sWithDefaultDc
        .addOtherDcMember(TestMember.withUniqueAddress(aa, Up, Set.empty, defaultDc))
    ...
    "add members from another same DC" in {
      sWithDefaultDc
        .addOtherDcMember(TestMember.withUniqueAddress(aa, Up, Set.empty, "dc-other"))
        .otherDcMembers should ===(Set(aa))
    }

    "remove the member" in {
      val s = sWithDefaultDc
        .addOtherDcMember(TestMember.withUniqueAddress(cc, Up, Set.empty, "dc-other"))
  ...
  "yield the correct indirectly-connected and unreachable nodes" in {
    ...
    val (s, events) = ReachabilityReporterState
      .updateReachabilities(
        CurrentClusterState(members = SortedSet(aaMember, bbMember, ccMember), seenBy = Set(aa.address, cc.address))
      )
      .run(sWithDefaultDc.withReachability(LithiumReachability(Set(aa), Map(bb -> Set(aa), cc -> Set(aa)))))
      .value
  ...
  "ignore nodes of other data-centers" in {
    ...
    val (s, events) = ReachabilityReporterState
      ...
      .run(
        sWithDefaultDc
          .addOtherDcMember(ccMember)
          .withReachability(LithiumReachability(Set(cc, dd), Map(bb -> Set(cc), aa -> Set(bb), cc -> Set(dd))))
      )
      .value
