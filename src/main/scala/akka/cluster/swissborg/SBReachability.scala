...
final case class SBReachability(private val r: Reachability) {
  def findUnreachableRecord(observer: UniqueAddress, subject: UniqueAddress): Option[Reachability.Record] =
  ...
  def observersGroupedByUnreachable: Map[UniqueAddress, Set[UniqueAddress]] = r.observersGroupedByUnreachable
}
